...
def run_pipeline(request: RunRequest, progress: Optional[ProgressCallback] = None) -> Dict[str, object]:
    ...
    total_usage = {"prompt_tokens": 0, "completion_tokens": 0, "total_tokens": 0, "truncated": False}
    ...
    for index, path in enumerate(request.inputs.files, start=1):
        ...
        if suffix not in TEXT_EXTENSIONS:
            ...
            if suffix == ".pdf":
                cleaned, restoration_record, usage = _restore_multimodal_pdf(path, instruction, profile, provider, progress)
            elif suffix == ".zip":
                cleaned, restoration_record, usage = _restore_multimodal_zip(path, instruction, profile, provider, progress)
            else:
                cleaned, restoration_record, usage = _restore_multimodal_image(path, instruction, profile, provider)
            ...
            cleaned, restoration_record, usage = _restore_text(raw_text, instruction, profile, provider)
            _add_usage(usage)
...
def _restore_text(raw_text: str, instruction: str, profile: WorkflowProfile, provider, media_path: Optional[Path] = None) -> tuple:
    ...
    for index, chunk in enumerate(chunks, start=1):
        ...
        result, usage = provider.restore_text(prompt, instruction, profile.model, media_path=media_path)
        if usage:
...
def _notify(progress: Optional[ProgressCallback], event: str, message: str, advance: int = 1) -> None:
    if progress:
...
def _parse_restoration_result(response: str, fallback_text: str) -> Dict[str, object]:
    ...
    if json_candidate:
        ...
        if isinstance(data, dict):
            restored_text = str(data.get("restored_text") or data.get("text") or data.get("output") or "").strip()
...
def _is_vision_model(model: str) -> bool:
    ...
    vision_keywords = ["vision", "-vl", "vl:", "gemma4", "gpt-4o", "gpt-5", "claude-3-5", "claude-sonnet-5", "claude-fable-5", "gemini-"]
...
def _task_text(raw_text: str, profile: WorkflowProfile) -> str:
    ...
    if execution_mode == "quality":
        ...
        depth_instruction = (
            "Perform a careful and thorough extraction of the text in the image.\n"
        )
    else:  # fast
        depth_instruction = (
            "Quickly extract the text from the image, focusing on speed and the most legible characters.\n"
        )
...
def _restore_multimodal_image(path: Path, instruction: str, profile: WorkflowProfile, provider) -> tuple:
...
def _extract_multimodal_text(response: str) -> str:
    ...
    if json_str:
        try:
            ...
            if isinstance(data, dict):
                text = str(data.get("restored_text") or data.get("text") or data.get("output") or "").strip()
...
def _restore_multimodal_pdf(path: Path, instruction: str, profile: WorkflowProfile, provider, progress: Optional[ProgressCallback]) -> tuple:
    ...
    try:
        ...
        if render.returncode != 0:
            raise RuntimeError(f"pdftoppm rendering failed (exit code {render.returncode}): {render.stderr}")
        ...
        for idx, page_img in enumerate(page_images, start=1):
            _notify(progress, "clean", f"Restoring text from {path.name} (page {idx}/{len(page_images)})", advance=1)
            prompt = _task_text("", profile)
            result, usage = provider.restore_text(prompt, instruction, profile.model, media_path=page_img)
            if usage:
                total_usage["prompt_tokens"] += usage.get("prompt_tokens", 0)
                total_usage["completion_tokens"] += usage.get("completion_tokens", 0)
                total_usage["total_tokens"] += usage.get("total_tokens", 0)
...
def _restore_multimodal_zip(path: Path, instruction: str, profile: WorkflowProfile, provider, progress: Optional[ProgressCallback]) -> tuple:
    ...
    try:
        ...
        for ext_file in sorted(extracted_files):
            ...
            if suffix in TEXT_EXTENSIONS:
                ...
                pdf_clean, pdf_rec, usage = _restore_multimodal_pdf(ext_file, instruction, profile, provider, progress)
                if usage:
                    total_usage["prompt_tokens"] += usage.get("prompt_tokens", 0)
                    total_usage["completion_tokens"] += usage.get("completion_tokens", 0)
                    total_usage["total_tokens"] += usage.get("total_tokens", 0)
                ...
                prompt = _task_text("", profile)
                result, usage = provider.restore_text(prompt, instruction, profile.model, media_path=ext_file)
                if usage:
                    total_usage["prompt_tokens"] += usage.get("prompt_tokens", 0)
                    total_usage["completion_tokens"] += usage.get("completion_tokens", 0)
                    total_usage["total_tokens"] += usage.get("total_tokens", 0)
